...
/**
 * Tenant-aware scheduler for Office 365 contact sync.
 */
public final class office365_contact_sync_scheduler {

    private static final Logger LOG = Logger.getLogger(office365_contact_sync_scheduler.class.getName());
    private static final long TICK_MS = 60_000L;
    private static final int DEFAULT_INTERVAL_MINUTES = 30;

    private static final class Holder {
        private static final office365_contact_sync_scheduler INSTANCE = new office365_contact_sync_scheduler();
    }

    public static office365_contact_sync_scheduler defaultService() {
        return Holder.INSTANCE;
    }

    private final Timer timer = new Timer("office365-contact-sync-timer", true);
    ...
    private final Office365ContactsSyncService syncService = new Office365ContactsSyncService();

    private office365_contact_sync_scheduler() {
    }
    ...
    public void startIfNeeded() {
        ...
        LOG.info("Office 365 contact sync scheduler started.");
    }
    ...
    private void tick() {
        ...
        try {
            ...
            LOG.log(Level.FINE, "Office 365 scheduler tick failed: " + safe(ex.getMessage()), ex);
        } finally {
    ...
    private void handleTenant(String tenantUuid) {
        ...
        if (!"true".equalsIgnoreCase(safe(cfg.get("office365_contacts_sync_enabled")))) {
        ...
        Thread worker = new Thread(() -> {
            try {
                syncService.syncContacts(tenantUuid, false);
            } catch (Exception ex) {
                LOG.log(Level.WARNING, "Scheduled Office 365 contact sync failed for tenant=" + tenantUuid + ": " + safe(ex.getMessage()), ex);
            } finally {
                inFlightTenants.remove(tenantUuid);
            }
        }, "office365-contact-sync-" + tenantUuid.replaceAll("[^A-Za-z0-9._-]", "_"));
