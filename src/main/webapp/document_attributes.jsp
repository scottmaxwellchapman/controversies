...
<%
  ...
  if ("POST".equalsIgnoreCase(request.getMethod())) {
    ...
    if ("save_document_attributes".equalsIgnoreCase(action)) {
      try {
        ...
        ArrayList<document_attributes.AttributeRec> rows = new ArrayList<document_attributes.AttributeRec>();
        for (int i = 0; i < n; i++) {
          ...
          rows.add(new document_attributes.AttributeRec(uuid, key, label, type, opts, req, en, ord, ""));
        }

        store.saveAll(tenantUuid, rows);
        response.sendRedirect(ctx + "/document_attributes.jsp?saved=1");
        return;
      } catch (Exception ex) {
        error = "Unable to save document attributes: " + safe(ex.getMessage());
      }
    }
  }

  if ("1".equals(request.getParameter("saved"))) message = "Document attribute definitions saved.";

  List<document_attributes.AttributeRec> attrs = new ArrayList<document_attributes.AttributeRec>();
...
<section class="card">
  <h1 style="margin:0;">Document Attributes</h1>
  <div class="meta" style="margin-top:4px;">Tenant-scoped document metadata fields. Use these on the Documents page to capture structured values.</div>
...
<section class="card" style="margin-top:12px;">
  <form class="form" method="post" action="<%= ctx %>/document_attributes.jsp">
    <input type="hidden" name="csrfToken" value="<%= esc(csrfToken) %>" />
    <input type="hidden" name="action" value="save_document_attributes" />
    ...
    <div class="table-wrap">
      <table class="table">
        ...
        <tbody id="docAttrRows">
        <%
          int idx = 0;
          for (int i = 0; i < attrs.size(); i++) {
            document_attributes.AttributeRec a = attrs.get(i);
            if (a == null) continue;
        %>
          <tr>
            <td>
            ...
            <td><input type="text" name="attr_key" value="<%= esc(safe(a.key)) %>" placeholder="document_source" /></td>
            <td>
            ...
            <td><button type="button" class="btn btn-ghost" onclick="removeDocAttrRow(this)">Remove</button></td>
          </tr>
        <%
            idx++;
          }
        ...
        %>
          <tr>
            ...
            <td><input type="text" name="attr_key" value="" placeholder="document_source" /></td>
            <td>
              <select name="attr_type">
    ...
    <div class="actions" style="display:flex; gap:10px; margin-top:10px;">
      <button type="button" class="btn btn-ghost" onclick="addDocAttrRow()">Add Attribute</button>
      <button type="submit" class="btn">Save Document Attributes</button>
    </div>
  </form>
</section>

<script>
  function addDocAttrRow() {
    var tbody = document.getElementById("docAttrRows");
    if (!tbody) return;
