...
<%
  ...
  if ("POST".equalsIgnoreCase(request.getMethod())) {
    ...
    if ("save_case_attributes".equalsIgnoreCase(action)) {
      try {
        ...
        String[] builtIn = request.getParameterValues("attr_builtin");
        ...
        n = Math.max(n, builtIn == null ? 0 : builtIn.length);
        n = Math.max(n, sort == null ? 0 : sort.length);

        ArrayList<case_attributes.AttributeRec> rows = new ArrayList<case_attributes.AttributeRec>();
        for (int i = 0; i < n; i++) {
          ...
          boolean bi = boolLike((builtIn != null && i < builtIn.length) ? builtIn[i] : "false");
          int ord = 0;
          try { ord = Integer.parseInt(safe((sort != null && i < sort.length) ? sort[i] : "").trim()); } catch (Exception ignored) {}

          rows.add(new case_attributes.AttributeRec(uuid, key, label, type, opts, req, en, bi, ord, ""));
        }

        store.saveAll(tenantUuid, rows);
        response.sendRedirect(ctx + "/case_attributes.jsp?saved=1");
        return;
      } catch (Exception ex) {
        error = "Unable to save case attributes: " + safe(ex.getMessage());
      }
    }
  }

  if ("1".equals(request.getParameter("saved"))) message = "Case attribute definitions saved.";

  List<case_attributes.AttributeRec> attrs = new ArrayList<case_attributes.AttributeRec>();
  try { attrs = store.listAll(tenantUuid); } catch (Exception ignored) {}
  int rows = Math.max(5, attrs.size() + 1);
%>

<jsp:include page="header.jsp" />

<section class="card">
  <h1 style="margin:0;">Case Attributes</h1>
  <div class="meta" style="margin-top:4px;">Tenant-scoped case metadata fields. Tokens are available as <code>{{case.key}}</code> and <code>{{kv.key}}</code>.</div>
...
<section class="card" style="margin-top:12px;">
  <form class="form" method="post" action="<%= ctx %>/case_attributes.jsp">
    <input type="hidden" name="csrfToken" value="<%= esc(csrfToken) %>" />
    <input type="hidden" name="action" value="save_case_attributes" />
      ...
      <table class="table">
        ...
        <%
          ...
          for (int i = 0; i < attrs.size(); i++) {
            case_attributes.AttributeRec a = attrs.get(i);
            if (a == null) continue;
            boolean bi = a.builtIn;
        %>
          <tr data-built-in="<%= bi ? "1" : "0" %>">
            <td>
              <input type="hidden" name="attr_uuid" value="<%= esc(safe(a.uuid)) %>" />
              <input type="hidden" name="attr_builtin" value="<%= bi ? "true" : "false" %>" />
            ...
            <td><input type="text" name="attr_key" value="<%= esc(safe(a.key)) %>" <%= bi ? "readonly" : "" %> /></td>
            <td>
            ...
            <td>
              <button type="button" class="btn btn-ghost" onclick="removeAttrRow(this)" <%= bi ? "disabled" : "" %>>Remove</button>
            </td>
          </tr>
        <%
            idx++;
        ...
        %>
          <tr data-built-in="0">
            <td>
              ...
              <input type="hidden" name="attr_builtin" value="false" />
              <input type="number" name="attr_sort" value="<%= (idx + 1) * 10 %>" min="0" step="10" />
            </td>
            <td><input type="text" name="attr_label" value="" /></td>
            <td><input type="text" name="attr_key" value="" placeholder="filing_court" /></td>
            <td>
              <select name="attr_type">
            ...
            <td><button type="button" class="btn btn-ghost" onclick="removeAttrRow(this)">Remove</button></td>
          </tr>
        <% } %>
        </tbody>
      </table>
    ...
    <div class="actions" style="display:flex; gap:10px; margin-top:10px;">
      <button type="button" class="btn btn-ghost" onclick="addAttrRow()">Add Attribute</button>
      <button type="submit" class="btn">Save Case Attributes</button>
    </div>
  </form>
</section>
...
<script>
  function addAttrRow() {
    var tbody = document.getElementById("attrRows");
    if (!tbody) return;
    var idx = tbody.querySelectorAll("tr").length + 1;
    var tr = document.createElement("tr");
    tr.setAttribute("data-built-in", "0");
    tr.innerHTML =
      '<td>' +
        '<input type="hidden" name="attr_uuid" value="" />' +
        '<input type="hidden" name="attr_builtin" value="false" />' +
  ...
  function removeAttrRow(btn) {
    ...
    if (!tr) return;
    if (tr.getAttribute("data-built-in") === "1") return;
    tr.remove();
  }
</script>
